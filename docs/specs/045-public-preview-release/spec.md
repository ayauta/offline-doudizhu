...
Approved: 2026-09-06
Decision: ADR 0012 and ADR 0014
...
- retained logcat and a screenshot for diagnosis.

The shell smoke owns exact-APK installation and Android-shell integration; it
does not locate Web DOM elements or inject raw Back key events. Playwright
remains the complete deterministic gameplay, embedded-entry, orientation, and
browser-interaction acceptance. Android source checks lock the Back and hardened
WebView contracts. A physical-device release checklist samples both landscape
rotations, lifecycle continuity, and two-press system Back without claiming that
manual evidence is automated.
...
5. runs the black-box shell smoke against that signed APK on API 36;
...
- Android lint/debug build and shell smoke pass locally when the corresponding
  SDK/emulator environment is available.
