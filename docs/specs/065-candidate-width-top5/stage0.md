...
**已定位机制**，不是回归：它的**最后一条**断言在若干次 `await` 之后才重新查询那条
**180 ms** 的 `hand-regroup` 动画。满负载时这些 `await` 超过 180 ms，动画已结束并被丢弃，
`getAnimations().find(...)` 返回 undefined → `frames` 为空 → `frames.length === 2` 为假。
空闲机器上稳定通过（单独重跑 5.2 s，整轮 `pnpm check` 也通过）。`src/` 零改动，无 UI 代码
被触碰。**记录在案而不是掩盖**：本轮 Stage 1/2 在同一台满载机器上跑，若它再次出现，
应读作采样竞态，而不是本案的回归。真正的修法是把那段断言合并成**一次** evaluate，
但那是改动验收测试，本轮不动。
