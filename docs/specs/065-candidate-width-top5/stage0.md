...
combined 模式在**一个进程里顺序跑两臂**。退休种子实测：**20 副 / 两臂 = 395 s**（满载下），
即两臂顺序 19.75 s/deal。按这个速率，Stage 2（1,200 组）在满载下要 **395 min**，
即使机器空闲也接近 2.5 h——**Stage 2 是本轮唯一的正式判定、不能半途而废**，这个余量不够。

并行化的常规做法是两臂各写各的文件，而那正是 Spec 064 死掉的形态（第一臂一结束，
磁盘上就有一份完整的单臂结果）。所以改成 `scripts/cf-top5-stage-run.mjs`：

* 两个 arm 作为**子进程**并行跑，各自把结果**打印到 stdout**，**不写任何文件**；
* 父进程把两份结果**收在内存里**，两臂都退出后**只写一次** combined 文件（同样 atomic）。

因此磁盘上**任何时刻都不存在**未完成的结果。实测（同样 20 副、同样满载）：
**并行 217 s vs 顺序 399 s = 1.84×**，且目录里只有最终那一个文件。

复算预算（机器空闲时约 3.5 s/deal/arm）：Stage 1 ≈ 12 min，Stage 2 ≈ 70 min。
语料 04:25 完成 → 流水线 ~04:50 → Stage 1 ~05:05 → Stage 2 ~06:15，
**距 08:30 约 2 h 余量**。
...
（另有一条 Playwright 动画测试在满负载下偶发失败：`production-table.spec.ts:738`。
**机制已用证据定位，不是回归**：它要断言一条 **180 ms、一次性**的 `hand-regroup` 动画
（180 ms 后浏览器就把它从 `getAnimations()` 摘掉）。测试先 `expect.poll` 等到动画存在，
再做一次浏览器往返去读它的 timing 与 keyframes。**满载时那次往返本身可能超过 180 ms**，
于是读到时动画已经消失，`null` → 断言失败。实测失败时的 received value 就是 `null`。

试过的修法：把 timing 与 keyframes 合并成**一次** evaluate，消除两次读之间的窗口。
**没有解决问题**（满载 6 次里仍失败 3 次）——因为残余竞态在「poll 成功」与「下一次往返」
之间，而这一段无法再压缩。**该改动已回滚**：它未被证明有效，就不该留在验收测试里。

结论：这个测试**本质上依赖时序**，在 16 核被 15 个语料分片占满时不可靠。它与本轮改动
无关（`src/` 零改动、未触碰 UI 代码）。**因此 `pnpm check` 的权威运行放在语料结束、
机器空闲之后**——满载下它不能作为 gate 使用，这一点比测试本身更值得记住。---
