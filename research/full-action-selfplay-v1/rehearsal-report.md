...
3. **"7 shells still running" 是 7 个空闲的 bash 等待循环**，不是计算。
   ...
   已在 2026-09-25 00:36 kill 掉 6 个（PID 3261756 / 3272779 / 3283492 / 3295121 / 3317486 / 3327511）。

   **更正（2026-09-25 11:04）**：当时漏掉第 7 个（PID 3306815），它又空转了 11 小时 16 分，
   直到被发现并 kill。当时的"核查"命令是
   `pgrep -fa "selfplay" | grep -v "pgrep\|bash -c source"` ——
   而这些等待循环**全部**是 `bash -c source …` 的形状，那个 `grep -v` 恰好把要找的行全部过滤掉，
   所以那次核查**结构上不可能**发现漏网者。**"没有任何后台计算被遗留"这句话当时是错的。**
   正确说法：它们自始至终 **CPU 0%、不做任何计算**，因此**不影响本报告的任何数字**；
   但"已清理干净"这个断言在当时是未经有效验证的。
   （与贯穿本线的纪律同一条：一个不会红的 guard 不是 guard。）
...
见 A.0（含 2026-09-25 11:04 的更正）。空闲等待 shell **曾经**有 7 个，第 7 个在其后 11 小时才被清掉；
它们不做计算，因此不影响任何测量。`pnpm check` 与所有测量都在真实运行上完成。
