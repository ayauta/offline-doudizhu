#!/usr/bin/env node
/**
 * Drives the Spec 065 top5 corpus: generate every shard in
 * parallel, then merge, then audit. Mirrors `cf-corpus.mjs`, pointed at this
 * round's universe, policy and salts.
 *
 *   source scripts/activate-toolchain.sh
 *   AI_CF_T5_JOBS=16 AI_CF_T5_POLICY_COMMIT=<sha> node scripts/cf-top5-corpus.mjs generate
 *   node scripts/cf-top5-corpus.mjs merge
 *   node scripts/cf-top5-corpus.mjs audit
 *
...
const BENCH = "benchmarks/cf-top5-corpus.test.ts";

const SHARD_DIR = process.env.AI_CF_T5_SHARD_DIR ?? join(ROOT, ".local", "cf-top5-shards");
const CORPUS_DIR = process.env.AI_CF_T5_CORPUS_DIR ?? join(ROOT, ".local", "cf-top5-corpus");
const ROWS_DIR = process.env.AI_CF_T5_ROWS_DIR ?? join(ROOT, ".local", "cf-top5-rows");

// Spec 065 §8. Written out here so a typo in an env var cannot silently move
// the round onto another pool.
const UNIVERSE_START = 140_001;
const UNIVERSE_END = 160_000;
const UNIVERSE_SIZE = UNIVERSE_END - UNIVERSE_START + 1;

const mode = process.argv[2] ?? "status";
const jobs = positiveInt("AI_CF_T5_JOBS", 1);
...
function policyCommit() {
  ...
  if (commit === undefined || commit === "") {
    ...
    throw new Error("AI_CF_T5_POLICY_COMMIT must name the driver commit.");
  }
  return commit;
}
...
async function generate() {
  ...
  if (process.env.AI_CF_T5_FRESH === "1") {
  ...
  const codes = await Promise.all(pending.map((window) => run(
    ...
    {
      AI_CF_T5_GENERATE: shardPath(window),
      AI_CF_T5_DEAL_START: String(window.start),
      AI_CF_T5_DEALS: String(window.count),
...
async function merge() {
  return run(["run", "--config", CONFIG, BENCH], {
    AI_CF_T5_MERGE: SHARD_DIR,
    AI_CF_T5_CORPUS_DIR: CORPUS_DIR,
    AI_CF_T5_POLICY_COMMIT: policyCommit(),
  });
}
