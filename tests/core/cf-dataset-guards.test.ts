...
/** The `default` tier is cheap and clock-free, which is what keeps the pure
 * checks inside `pnpm check` fast. `master` is used only where the production
 * candidate set is the thing under test. */
const LIGHT_TIERS: CfSeatTiers = Object.freeze({
  human: "default",
  "ai-one": "default",
  "ai-two": "default",
});

function studiedTiers(studiedSeat: Seat): CfSeatTiers {
...
function zeroCounters(): CfPolicyCounters {
 ...
 * holding the studied seat's hand and every public fact fixed. The landlord's
 * hand keeps the revealed bottom cards and the table keeps the cards already
 * played, so the alternative world is one no public statement contradicts.
 */
function redealHidden(state: PlayingState, studied: Seat, seed: number): PlayingState {
 ...
 * written against an arbitrary root can pass on any rule at all. The caller
 * asserts the precondition.
 */
function wideFarmerRoot(): Readonly<{
  state: GameState;
  counters: CfPolicyCounters;
  gameSeed: number;
  seat: Seat;
